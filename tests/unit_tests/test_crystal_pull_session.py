...
def test_pre_draw_sc_ops_df():
    ...
    test_data_dict = {
        ...
        "expected_pre_draw_stamp_card_values": [0, 1, 6, 9, 3, 7, 9, 9, 10, 11, 0],
        "expected_pre_draw_stamp_card_indices": [0, 0, 0, 0, 1, 1, 1, 2, 2, 2, 3],
        "expected_post_draw_stamp_card_values": [1, 6, 9, 3, 7, 9, 9, 10, 11, 0, 0],
        "expected_post_draw_stamp_card_indices": [
            ...
            2,
            3,
            4,  # Repeat of "page_ex" will be captured in test for expected rules
        ],
...
def assert_all_cases_pass(
    test_case_df, expected_col, output_col, input_col=None, print_failures_only=False
):
    ...
    filtered_test_case_df["result"] = filtered_test_case_df.apply(
        (lambda row: "PASS" if row[expected_col] == row[output_col] else "FAIL"), axis=1
    )
...
def generate_pre_draw_sc_ops_output_df(
    test_crystal_pull_session, test_pre_draw_sc_ops_df, column_suffix
):
    """
    ...
    for value in test_pre_draw_sc_ops_df["input_ten_draw_stamp_values"]:
        output_dict["output_pre_draw_stamp_card_indices"].append(
            test_crystal_pull_session.current_stamp_card_index
        )
        output_dict["output_pre_draw_stamp_card_values"].append(
            test_crystal_pull_session.current_stamp_card.current_stamp_value
        )

        test_crystal_pull_session.pre_draw_stamp_card_operations(
            predetermined_stamp_value=value
        )

        output_dict["output_rules_for_draw"].append(
            test_crystal_pull_session.rules_for_next_ten_draw
        )

        test_crystal_pull_session.perform_ten_draw()  # Resets rules for next draw, and it's what always happens next anyway

        output_dict["output_post_draw_stamp_card_values"].append(
            test_crystal_pull_session.current_stamp_card.current_stamp_value
        )
        output_dict["output_post_draw_stamp_card_indices"].append(
            test_crystal_pull_session.current_stamp_card_index
        )
    ...
    filtered_df = test_pre_draw_sc_ops_df[
        ["input_ten_draw_stamp_values"]
        + [col for col in test_pre_draw_sc_ops_df.columns if column_suffix in col]
    ]
...
def test_determine_stamp_value_for_ten_draw(test_crystal_pull_session):
    ...
    test_dsv_df["outputs"] = test_dsv_df.apply(
        (
            lambda row: test_crystal_pull_session.determine_stamp_value_for_ten_draw(
                predetermined_int=row["inputs"]
            )
        ),
        axis=1,
    )
    ...
    test_dsv_df["result"] = test_dsv_df.apply(
        (
            lambda row: (
                "PASS"
                if row["outputs"] == row["expected_incremental_value"]
                else "FAIL"
            )
        ),
        axis=1,
...
def test_move_to_next_stamp_card(test_crystal_pull_session):
    ...
    for stamp_card_key in expected_stamp_card_list_keys:
        expected_stamp_cards.append(
            StampCard(
                test_crystal_pull_session.stamp_cards_list[stamp_card_key]
            ).position_and_rule_df.to_dict()
        )
    ...
    assert_all_cases_pass(
        test_case_df=test_move_df, expected_col="expected", output_col="outputs"
    )


def test_pre_draw_stamp_card_operations_pre_draw_stamp_card_values(
    test_crystal_pull_session, test_pre_draw_sc_ops_df
):
    """
    ...
    test_df = generate_pre_draw_sc_ops_output_df(
        test_crystal_pull_session, test_pre_draw_sc_ops_df, column_suffix=column_suffix
    )

    assert_all_cases_pass(
        test_case_df=test_df,
        expected_col="expected_" + column_suffix,
        output_col="output_" + column_suffix,
    )


def test_pre_draw_stamp_card_operations_pre_draw_stamp_card_indices(
    test_crystal_pull_session, test_pre_draw_sc_ops_df
):
    """
...
def test_pre_draw_stamp_card_operations_post_draw_stamp_card_values(
    test_crystal_pull_session, test_pre_draw_sc_ops_df
):
    """
...
def test_pre_draw_stamp_card_operations_post_draw_stamp_card_indices(
    test_crystal_pull_session, test_pre_draw_sc_ops_df
):
    """
...
def test_pre_draw_stamp_card_operations_rules_for_draw(
    test_crystal_pull_session, test_pre_draw_sc_ops_df
):
    """
